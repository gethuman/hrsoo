        ...
        if (token.type === 'ampm') {
            ...
            if (prevToken.type === 'time') {
                ...
                utils.log('Previous token is ' + prevToken.type + ' ' +
                            prevToken.value +  ' so ignore timzone ' + token.value, state);
            }
...
function getDayTimes(state) {
    ...
    if (Object.keys(dayTimes).length === 7 && isEveryDayAllTime) {
        return { everyDayAllTime: true };
    }
    else {
        dayTimes.timezone = state.timezone;
        return dayTimes;
    }
}
