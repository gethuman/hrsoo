...
var operations = {
    through: function through(state, index) {
        ...
        if (prev.type === 'day') {
            ...
            if (startDayIdx < 0) {
                utils.log('Day ' + prev.value + ' does not exist', state);
            }
            else if (endDayIdx < 0) {
                utils.log('Day ' + prev.value + ' does not exist', state);
            }
            else if (startDayIdx >= endDayIdx) {
                utils.log(prev.value + ' must be before ' + next.value, state);
            }
            else {
                days = [];
            ...
            utils.log('Invalid type ' + prev.type + ' for through operation');
        }
    ...
    for (i = (state.tokens.length - 1); i >= 0; i--) {
        ...
        if (token.type === 'timezone') {
            if (timezone && token.value !== timezone) {
                utils.log('You cannot use multiple timezones: ' + timezone + ' and ' + token.value);
            }
            else {
    ...
    for (i = (state.tokens.length - 1); i > 0; i--) {  // not a mistake > 0 because ampm shouldn't be first token
        ...
        if (token.type === 'ampm') {
            ...
            if (prevToken.type === 'time') {
                ...
                utils.log('Invalid token ' + prevToken.type + ' ' +
